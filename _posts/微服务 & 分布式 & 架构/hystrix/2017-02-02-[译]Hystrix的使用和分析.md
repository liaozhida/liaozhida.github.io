...
```
// Whether fuse opening in the whole statistical time value, the default 20 seconds. It is 10 seconds at least 20 request, to play the role of fuse
private final HystrixProperty<Integer> circuitBreakerRequestVolumeThreshold; 
//Fuse the default working time, default: 5 seconds. Fuse interrupt request 5 seconds after entering the half open state, part of flow past a retry
private final HystrixProperty<Integer> circuitBreakerSleepWindowInMilliseconds; 
//Whether to enable the fuse, the default true. boot
private final HystrixProperty<Boolean> circuitBreakerEnabled; 
//Default: 50%. When the error rate is more than 50% fuse start.
private final HystrixProperty<Integer> circuitBreakerErrorThresholdPercentage;
//Whether forced open the fuse block all requests, default: false, don't open
private final HystrixProperty<Boolean> circuitBreakerForceOpen; 
//Whether to allow the fuse to ignore the error, the default false, do not open
...
每次调用的第一步 1: 创建一个 HystrixCommand, 具体的调用被包装在  run () 方法中.

2: 使用 execute (/queue) ，分别是同步或异步的方式

3: 查看熔断器是否打开, 是的话跳到 step 8, 降级策略, 不是就进入下一步流程.

4：thread pool / queue / signal 资源是否耗尽, 如果是降级到 step 8, 否则下一步流程

5: HystrixCommand 具体的业务操作
...
7: 计算熔断的健康状态,  所有这些状态(success, failure, rejection, timeout) 都会被报告给熔断器, 熔断器基于这些数据来决定下一步的走向

8:getFallback()Degradation of logic.
下面四种情况会触发 getFallback 调用:
 (1):run()The method throws an non HystrixBadRequestException anomalies.
 (2):run()The method call timeout
 (3):Fuse opening to intercept calls
 (4):The thread pool / queue / signal whether the amount of running full

8a: 没有实现getFallback 方法的 Command 会直接抛出 exception
 
8b: 降级操作的逻辑调用是成功的就直接返回  

8c: 降级操作调用失败就抛出 exception
