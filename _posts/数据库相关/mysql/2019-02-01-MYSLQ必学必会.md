...
- index merge ：同一个表的多个索引的范围扫描可以对结果进行合并，合并方式分为三种：union(), intersection, 以及它们的组合
...
order by时`select *`是一个大忌，只query需要的字段:当query的字段大小总和小于max_length_for_sort_data而且排序字段不是text或blob类型时，会用改进后的算法–单路排序，否则用老算法–多路排序
